 ...
 *
 * 核心逻辑：完全基于价格行为，不依赖趋势指标。
 *
 * 入场条件（两步必须同时满足）：
 *   Step 1【触位】：最近 3 根 K 线中，至少一根的 High/Low 触及关键 S/R 位（容差 0.3%）
 *   Step 2【拒绝】：该触位 K 线（或前后临近 K 线）形成"影线拒绝"形态：
 *     做空：上影线占振幅 ≥ 35% + 上影线 > 下影线 + 收盘在中线以下
 *     做多：下影线占振幅 ≥ 35% + 下影线 > 上影线 + 收盘在中线以上
 *
 * SL/TP：
 *   做空 SL = 拒绝 K 线最高价 + 0.3×ATR（止损在影线极值上方）
 *   做多 SL = 拒绝 K 线最低价 - 0.3×ATR（止损在影线极值下方）
 *   做空 TP = 最近支撑位（若不足 1.5×risk 则 fallback 到 2×ATR）
 *   做多 TP = 最近阻力位（若不足 1.5×risk 则 fallback 到 2×ATR）
 *   最低赔率要求：1.5:1
 *
...
public class SRRejectionScalpingStrategy implements Strategy {
    ...
    /** 触位容差（0.2%，缩紧：价格必须真正贴近 S/R 位，减少假触位） */
    private static final double TOUCH_THRESHOLD_PCT = 0.002;
    ...
    public TradingSignal generateSignal(MarketContext context) {
        ...
        if (resistance != null) {
            ...
            if (bearish.found) {
                // StochRSI 过滤：K 须在超买区（>75），动量支持做空
                if (stochRSI.getK() < STOCH_OVERBOUGHT_THRESHOLD) {
                    log.info("[SRRejection] 做空被 StochRSI 过滤 K={} < {}（动量未超买）",
                            String.format("%.1f", stochRSI.getK()), STOCH_OVERBOUGHT_THRESHOLD);
                } else {
                    double sl = bearish.wickExtreme + atr * SL_ATR_BUFFER;
        ...
        if (support != null) {
            ...
            if (bullish.found) {
                // StochRSI 过滤：K 须在超卖区（<25），动量支持做多
                if (stochRSI.getK() > STOCH_OVERSOLD_THRESHOLD) {
                    log.info("[SRRejection] 做多被 StochRSI 过滤 K={} > {}（动量未超卖）",
                            String.format("%.1f", stochRSI.getK()), STOCH_OVERSOLD_THRESHOLD);
                } else {
                    double sl = bullish.wickExtreme - atr * SL_ATR_BUFFER;
